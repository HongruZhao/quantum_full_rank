...
spectral_projectors <- function(B, tol = 1e-10) {
  ...
  H <- hermitianize(B)
  is_real <- all(Im(H) == 0)
  H_use <- if (is_real) Re(H) else H
  ## Use the symmetric path only for real matrices to avoid crashes on complex input.
  ee <- eigen(H_use, symmetric = is_real)
  vals <- Re(ee$values)
  vecs <- ee$vectors
...
## ------------------------------------------------------------
## Core utilities for adaptive A-optimal design simulations.
## ------------------------------------------------------------
use_ggplot2 <- requireNamespace("ggplot2", quietly = TRUE)

make_ab_index <- function(Q_list) {
  k <- length(Q_list)
  ...
  for (a in seq_len(k)) {
    r_a <- length(Q_list[[a]])
    ab_row[[a]] <- (row + 1L):(row + r_a)
    df_list[[a]] <- data.frame(a = rep(a, r_a),
                               b = seq_len(r_a),
                               row = ab_row[[a]])
    row <- row + r_a
  }
  ab_df <- do.call(rbind, df_list)
  list(ab_df = ab_df, ab_row = ab_row, k = k, M = row)
}

pauli_from_vec <- function(v, sigmas) {
  v[1] * sigmas[[1]] + v[2] * sigmas[[2]] + v[3] * sigmas[[3]]
}

make_pvm_setting <- function(v, sigmas, N) {
  I_N <- diag(N)
  B <- pauli_from_vec(v, sigmas)
  list(0.5 * (I_N + B), 0.5 * (I_N - B))
}

make_measurement_library <- function(lib_id, N, sigmas, seed_lib4 = NULL) {
  ...
  if (lib_id == 1) {
    Q_list <- lapply(seq_len(3), function(j) make_pvm_setting(c(0, 0, 0) + diag(3)[j, ], sigmas, N))
    lib_label <- "Library 1: Pauli PVMs"
  } else if (lib_id == 2) {
    axes <- rbind(
      c(1, 0, 0),
      c(0, 1, 0),
    ...
    Q_list <- lapply(seq_len(nrow(axes)), function(j) make_pvm_setting(axes[j, ], sigmas, N))
    lib_label <- "Library 2: Nine-axis PVMs"
  } else if (lib_id == 3) {
    pauli6 <- list()
    for (j in seq_len(3)) {
      B <- sigmas[[j]]
      pauli6[[length(pauli6) + 1]] <- (1 / 6) * (I_N + B)
      pauli6[[length(pauli6) + 1]] <- (1 / 6) * (I_N - B)
    }
    tetra <- rbind(
      c(1, 1, 1) / sqrt(3),
    ...
    tetra_list <- lapply(seq_len(nrow(tetra)), function(j) {
      (1 / 4) * (I_N + pauli_from_vec(tetra[j, ], sigmas))
    })
    Q_list <- list(pauli6, tetra_list)
    lib_label <- "Library 3: Pauli-6 + Tetrahedral POVM"
  } else if (lib_id == 4) {
    ...
    Q_list <- with_seed(seed_lib4, {
      lapply(seq_len(4), function(j) make_pvm_setting(rand_vec(), sigmas, N))
    })
    lib_label <- "Library 4: Random PVMs"
  } else {
    stop("Unknown library id.")
  }

  ab <- make_ab_index(Q_list)
  SC <- build_Sab_cab(sigmas, Q_list, N, ab$ab_df)
  list(Q_list = Q_list,
       ab_df = ab$ab_df,
       ab_row = ab$ab_row,
       S_ab = SC$S_ab,
...
apply_eig_floor <- function(rho, eta) {
  if (is.null(eta) || eta <= 0) return(rho)
  H <- hermitianize(rho)
  is_real <- all(Im(H) == 0)
  ee <- eigen(if (is_real) Re(H) else H, symmetric = is_real)
  vals <- pmax(Re(ee$values), eta)
  rho_floor <- ee$vectors %*% diag(vals) %*% Conj(t(ee$vectors))
  rho_floor / traceC(rho_floor)
}

build_task_metric <- function(obs_list, sigmas) {
  d <- length(sigmas)
  G <- matrix(0, d, d)
  for (O in obs_list) {
    O <- hermitianize(O)
    o_vec <- vapply(sigmas, function(S) as.numeric(Re(traceC(S %*% O))), numeric(1))
    G <- G + 0.25 * (o_vec %*% t(o_vec))
  }
  G
}

bures_metric_matrix <- function(theta_hat, sigmas, N, params) {
  d <- length(sigmas)
  rho <- rho_of_theta(theta_hat, sigmas, N)
  rho <- apply_eig_floor(rho, params$eta_mle)
  I_N <- diag(N)
  A <- kronecker(I_N, rho) + kronecker(t(rho), I_N)
  if (!is.null(params$ridge_inv) && params$ridge_inv > 0) {
    A <- A + params$ridge_inv * diag(nrow(A))
  }

  G <- matrix(0, d, d)
  for (k in seq_len(d)) {
    rhs <- 2 * as.vector(sigmas[[k]])
    vecY <- tryCatch(solve(A, rhs), error = function(e) NULL)
    if (is.null(vecY)) {
      return(0.5 * diag(d))
    }
    Y <- matrix(vecY, N, N)
    for (j in seq_len(d)) {
      G[j, k] <- as.numeric(Re(traceC(sigmas[[j]] %*% Y))) / 16
    }
  }
  (G + t(G)) / 2
}

L_tr_mu <- function(theta_prime, theta_ref, sigmas, N, mu) {
  rho_p <- rho_of_theta(theta_prime, sigmas, N)
  rho_r <- rho_of_theta(theta_ref, sigmas, N)
  delta <- hermitianize(rho_p - rho_r)
  is_real <- all(Im(delta) == 0)
  ee <- eigen(if (is_real) Re(delta) else delta, symmetric = is_real)
  vals <- Re(ee$values)
  psi <- mu * log(2 * cosh(vals / mu))
  d_tr <- 0.5 * sum(psi)
  d_tr^2
}

numeric_hessian <- function(f, x, h = 1e-4) {
  d <- length(x)
  H <- matrix(0, d, d)
  for (i in seq_len(d)) {
    for (j in seq_len(d)) {
...
logcosh_metric_matrix <- function(theta_hat, sigmas, N, params) {
  mu <- params$mu_logcosh
  f <- function(theta_prime) L_tr_mu(theta_prime, theta_hat, sigmas, N, mu)
  if (requireNamespace("numDeriv", quietly = TRUE)) {
    H <- numDeriv::hessian(f, theta_hat)
  } else {
    H <- numeric_hessian(f, theta_hat)
  }
  (H + t(H)) / 2
}

make_metric <- function(task_id, N, sigmas, params) {
  task_id <- as.character(task_id)
  d <- length(sigmas)
  if (task_id == "Frob") {
    G_const <- 0.5 * diag(d)
    G_func <- function(theta_hat) G_const
    task_label <- "Frobenius"
  } else if (task_id == "Bures") {
    G_func <- function(theta_hat) bures_metric_matrix(theta_hat, sigmas, N, params)
    task_label <- "Bures"
  } else if (task_id == "LogCosh") {
    G_func <- function(theta_hat) logcosh_metric_matrix(theta_hat, sigmas, N, params)
    task_label <- "LogCosh"
  } else if (task_id == "TaskObs") {
    G_const <- build_task_metric(params$obs_list, sigmas)
    G_func <- function(theta_hat) G_const
    task_label <- "TaskObs"
  } else {
    stop("Unknown task id.")
  }
  list(G_func = G_func, task_label = task_label)
}

fit_theta_cvxr_eta <- function(N, sigmas, S_ab, c_ab, Nab,
                               solver = c("MOSEK", "ECOS", "SCS"),
                               eps_log = 1e-12, eta_mle = 0, verbose = FALSE) {
  if (is.null(eta_mle) || eta_mle <= 0) {
    return(fit_theta_cvxr(N, sigmas, S_ab, c_ab, Nab, solver = solver,
                          eps_log = eps_log, verbose = verbose))
  }
  d <- length(sigmas)
  theta <- Variable(d)
  S_const <- as.matrix(S_ab)
  ...
  obj <- -sum_entries(Nab * log(p + eps_log))
  constr <- list(rho == A_affine, p >= 0, rho %>>% (eta_mle * SI))
  prob <- Problem(Minimize(obj), constr)
...
select_next_setting_A_weighted <- function(theta_hat, counts_by_a,
                                           S_ab, c_ab, ab_df,
                                           G_mat,
                                           method = c("exact", "GI1"),
                                           ridge = 1e-8, eps = 1e-12) {
  method <- match.arg(method)
  tf <- total_fisher_info(theta_hat, counts_by_a, S_ab, c_ab, ab_df,
                          eps = eps, check_positive = FALSE)
  I_total <- tf$I_total
  I_list <- tf$I_list
  d <- ncol(S_ab)
  ...
  if (method == "exact") {
    for (nm in names(I_list)) {
      M <- I_total + I_list[[nm]] + ridge * diag(d)
      Minv <- tryCatch(solve(M), error = function(e) NULL)
      scores[[nm]] <- if (is.null(Minv)) -Inf else -sum(diag(G_mat %*% Minv))
    }
  } else {
    M <- I_total + ridge * diag(d)
    Minv <- tryCatch(solve(M), error = function(e) NULL)
    if (is.null(Minv)) {
      scores[] <- -Inf
    } else {
      for (nm in names(I_list)) {
        scores[[nm]] <- sum(diag(G_mat %*% Minv %*% I_list[[nm]] %*% Minv))
      }
    }
  }

  best <- as.integer(names(scores)[which.max(scores)])
  list(setting = best, scores = scores, I_total = I_total, I_list = I_list)
}

proxy_risk_A <- function(theta_hat, counts_by_a, S_ab, c_ab, ab_df, G_mat, ridge = 1e-8) {
  tf <- total_fisher_info(theta_hat, counts_by_a, S_ab, c_ab, ab_df,
                          eps = 1e-12, check_positive = FALSE)
  J <- tf$I_total
  d <- ncol(S_ab)
  Minv <- tryCatch(solve(J + ridge * diag(d)), error = function(e) NULL)
  if (is.null(Minv)) return(NA_real_)
  sum(diag(G_mat %*% Minv))
}

resolve_true_state <- function(theta_true, rho_true, params, sigmas, seed) {
  if (!is.null(theta_true)) {
    rho_true <- rho_of_theta(theta_true, sigmas, params$N)
  } else if (!is.null(rho_true)) {
    theta_true <- theta_from_rho(rho_true, sigmas)
  } else if (params$true_state_mode == "random") {
    rho_true <- random_density(params$N, seed = seed)
    theta_true <- theta_from_rho(rho_true, sigmas)
  } else {
    theta_true <- params$theta_true
    rho_true <- rho_of_theta(theta_true, sigmas, params$N)
  }
  list(theta_true = theta_true, rho_true = rho_true)
}

run_one_rep <- function(policy, n_total, n_init,
                        lib_obj, metric_obj,
                        theta_true = NULL, rho_true = NULL,
                        params, seed) {
  set.seed(seed)
  sigmas <- lib_obj$sigmas
  Q_list <- lib_obj$Q_list
  ab_df <- lib_obj$ab_df
  ab_row <- lib_obj$ab_row
  S_ab <- lib_obj$S_ab
  c_ab <- lib_obj$c_ab
  k <- lib_obj$k
  M <- nrow(ab_df)
  ...
  true_state <- resolve_true_state(theta_true, rho_true, params, sigmas, seed + 1000)
  theta_true <- true_state$theta_true
  rho_true <- true_state$rho_true

  p_list_true <- born_probs_list(rho_true, Q_list)
  Nab <- numeric(M)
  a_hist <- integer(n_total)
  b_hist <- integer(n_total)
  risk_vec <- numeric(n_total)
  theta_path <- matrix(NA_real_, nrow = n_total, ncol = d)

  theta_hat <- rep(0, d)
  init_settings <- integer(0)
  if (!is.null(params$init_settings_by_library)) {
    init_settings <- params$init_settings_by_library(lib_obj$lib_id)
  }
  if (length(init_settings) > 0) {
    if (any(init_settings < 1 | init_settings > k)) {
      stop("init_settings_by_library returned invalid setting indices.")
    }
    n_init <- max(n_init, length(init_settings))
  }

  for (n in seq_len(n_total)) {
    if (n <= length(init_settings)) {
      a_next <- init_settings[n]
    } else if (n <= n_init) {
      a_next <- sample.int(k, size = 1)
    } else if (policy == "Uniform") {
      a_next <- sample.int(k, size = 1)
    } else {
      G_mat <- metric_obj$G_func(theta_hat)
      counts_by_a <- counts_by_setting(Nab, ab_df)
      method <- if (policy == "Exact") "exact" else "GI1"
      sel <- select_next_setting_A_weighted(theta_hat, counts_by_a, S_ab, c_ab, ab_df,
                                            G_mat, method = method,
                                            ridge = params$ridge_inv,
                                            eps = params$eps_prob)
      a_next <- sel$setting
    }

    b_next <- sample.int(length(p_list_true[[a_next]]), size = 1, prob = p_list_true[[a_next]])
    Nab[ab_row[[a_next]][b_next]] <- Nab[ab_row[[a_next]][b_next]] + 1
    a_hist[n] <- a_next
    b_hist[n] <- b_next

    do_refit <- params$refit_every > 0 && (n %% params$refit_every == 0)

    if (do_refit) {
      fit <- fit_theta_cvxr_eta(params$N, sigmas, S_ab, c_ab, Nab,
                                solver = params$solver,
                                eps_log = params$eps_log,
                                eta_mle = params$eta_mle,
                                verbose = FALSE)
      if (is.finite(sum(fit$theta_hat))) {
        theta_hat <- fit$theta_hat
      }
    }
    theta_path[n, ] <- theta_hat

    counts_by_a <- counts_by_setting(Nab, ab_df)
    G_true <- metric_obj$G_func(theta_true)
    risk_vec[n] <- proxy_risk_A(theta_true, counts_by_a, S_ab, c_ab, ab_df,
                                G_true, ridge = params$ridge_inv)
  }
  ...
  colnames(theta_path) <- paste0("theta_", seq_len(d))
  cbind(out_df, theta_path)
}

run_mc_study <- function(lib_id, task_id, params) {
  if (as.integer(lib_id) == 3 && task_id == "TaskObs") {
    stop("TaskObs is excluded for library 3.")
  }

  bas <- build_suN_basis(params$N)
  sigmas <- bas$sigmas
  lib_obj <- make_measurement_library(lib_id, params$N, sigmas, params$seed_lib4)
  metric_obj <- make_metric(task_id, params$N, sigmas, params)
  init_settings <- if (!is.null(params$init_settings_by_library)) {
    params$init_settings_by_library(lib_id)
  } else {
    integer(0)
  }
  n_init <- max(params$n_init_by_library(lib_id), length(init_settings))

  policies <- c("Exact", "GI1", "Uniform")
  mc_reps <- params$mc_reps

  theta_true_list <- vector("list", mc_reps)
  if (params$true_state_mode == "random") {
    for (r in seq_len(mc_reps)) {
      rho_r <- random_density(params$N, seed = params$seed_base + 90000 + r)
      theta_true_list[[r]] <- theta_from_rho(rho_r, sigmas)
    }
  } else {
    for (r in seq_len(mc_reps)) theta_true_list[[r]] <- params$theta_true
  }

  raw_list <- list()
  idx <- 1L
  for (p_idx in seq_along(policies)) {
    policy <- policies[p_idx]
    for (r in seq_len(mc_reps)) {
      seed <- params$seed_base + 10000 * p_idx + r
      rep_df <- run_one_rep(policy, params$n_total, n_init,
                            lib_obj, metric_obj,
                            theta_true = theta_true_list[[r]],
  ...
  raw_df$lib_id <- lib_id
  raw_df$task_id <- task_id

  summary_df <- aggregate(risk ~ n + policy, data = raw_df, FUN = mean)
  names(summary_df)[names(summary_df) == "risk"] <- "mean_risk"
  summary_df$lib_id <- lib_id
  summary_df$task_id <- task_id
  summary_df$lib_label <- lib_obj$lib_label
  summary_df$task_label <- metric_obj$task_label
  plot_min_n <- if (!is.null(params$plot_min_n)) params$plot_min_n else NULL
  attr(summary_df, "plot_min_n") <- plot_min_n

  list(raw = raw_df, summary = summary_df)
}

plot_proxy_risk <- function(summary_df, out_file, title = NULL, min_n = NULL, rescale_by_n = FALSE) {
  if (is.null(min_n)) {
    min_n <- attr(summary_df, "plot_min_n")
  }
  plot_df <- summary_df
  if (!is.null(min_n)) {
    plot_df <- plot_df[plot_df$n >= min_n, , drop = FALSE]
  }
  if (rescale_by_n) {
    plot_df$mean_risk <- plot_df$mean_risk * plot_df$n
    y_label <- "Mean proxy risk n * R_n(G)"
  } else {
    y_label <- "Mean proxy risk R_n(G)"
  }
  if (is.null(title)) {
    title <- paste(plot_df$lib_label[1], plot_df$task_label[1], sep = " | ")
  }
  if (use_ggplot2) {
    p <- ggplot2::ggplot(plot_df, ggplot2::aes(x = n, y = mean_risk, color = policy)) +
      ggplot2::geom_line(linewidth = 1) +
      ggplot2::labs(title = title, x = "Sample size n", y = y_label, color = "Policy") +
      ggplot2::theme_minimal(base_size = 12)
    ggplot2::ggsave(out_file, p, width = 7, height = 4.5, dpi = 150)
  } else {
    png(out_file, width = 1050, height = 675)
    on.exit(dev.off())
    policies <- unique(plot_df$policy)
    cols <- c("#1b9e77", "#d95f02", "#7570b3")
    plot(NA, xlim = range(plot_df$n), ylim = range(plot_df$mean_risk, na.rm = TRUE),
         xlab = "Sample size n", ylab = y_label, main = title)
    for (i in seq_along(policies)) {
      df <- plot_df[plot_df$policy == policies[i], ]
      lines(df$n, df$mean_risk, col = cols[i], lwd = 2)
    }
    legend("topright", legend = policies, col = cols[seq_along(policies)], lwd = 2, bty = "n")
  }
}

plot_proxy_risk_rescaled <- function(summary_df, out_file, title = NULL, min_n = NULL) {
  plot_proxy_risk(summary_df, out_file, title = title, min_n = min_n, rescale_by_n = TRUE)
}
